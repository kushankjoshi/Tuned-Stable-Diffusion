...
def main(args):
    ...
    # Add progress bar for overall training
    progress_bar = tqdm(range(args.max_train_steps), desc="Training Progress", disable=not accelerator.is_local_main_process)
    global_step = 0
    ...
    for epoch in range(args.num_train_epochs):
        ...
        completed_epochs += 1
        logger.info(f"Completed epoch {completed_epochs}/{args.num_train_epochs}")

        accelerator.wait_for_everyone()

    save_weights(global_step)

    logger.info(f"Training completed after {completed_epochs} epochs")
